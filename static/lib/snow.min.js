var snow=snow||{};(function(S,d3){S.tools=S.tools||{};S.tools.map2tsv=function(d,orderedLabels){var labels=Object.keys(d)
...
url+=qs.join('&');jsonp(url);}}(snow,d3))
var snow=snow||{};(function(S,d3){S.tsv=function(url,callback){$.ajax({type:"GET",url:url,dataType:"text",success:function(fdata){var x=fdata.split("\n");var cols=x[0].split("\t");var data={}
data["table"]={}
data["table"]["cols"]=[]
data["table"]["rows"]=[]
...
j+=1;}}
callback(data)}});}}(snow,d3))
var snow=snow||{};(function(S,d3){S.config=function(){var data;var typeMap={"i":"number","a":"text","s":"text","f":"text","index":"text","color":"color","bool":"checkbox"}
...
return typeMap[type]})
ti.attr("value",function(d){if(data[d].v!=undefined){return data[d].v}
return data[d].d})
ti.property("disabled",function(d){if(data[d].disabled){return data[d].disabled}
return false})
var tc=tv.selectAll("input[type='checkbox']")
tc.property("checked",function(d){if(data[d].v==undefined){return data[d].d}
return data[d].v})
...
tm.exit().remove()
tm.enter().append("div")
var tclist=tm.selectAll(".checklist").data(function(d){return data[d].m})
tclist.exit().remove()
tclist.enter().append("div").attr("class","checklist")
tclist.selectAll("input").remove();var checkboxs=tclist.append("input").attr("type","checkbox").attr("value",function(d){return d3.select(this.parentNode).datum()})
...
tclist.selectAll("text").remove();tclist.append("text").text(function(d){return d})
...
data[d].v=[]})
tclist.selectAll('input:checked').attr("selected",function(d){var key=d3.select(this.parentNode.parentNode).datum()
data[key].v.push(d)})
callback(data)})}
...
var snow=snow||{};(function(S,d3){S.panel=S.panel||{};S.tools=S.tools||{};S.tools.ParseIndexes=function(s){var k=s.split(",")
...
return a}
S.panel.general=function(cb){var ViewPane,CfgPane;var Content;var NavTabs;var NavView,NavCfg;var Config;var Div;var Title=""
...
var initView=false
var panel=function(selection){var Uid;Div=selection
...
title.enter().append("li").attr("uid",function(d){return S.tools.randomString(8);})
...
NavCfg.selectAll("a").data([{}]).enter().append("a").attr("href","#"+Uid+"_cfg").attr("data-toggle","tab").text("Config")
...
if(initView){var wait=ViewPane.selectAll(".overlay").data([0])
wait.enter().append("div")
wait.attr("class","overlay").append("i").attr("class","fa fa-refresh fa-spin")
wait.exit().remove();ViewPane.call(chart)
wait.remove()}
...
panel.update=function(selection){var wait=ViewPane.selectAll(".overlay").data([0])
wait.enter().append("div")
wait.attr("class","overlay").append("i").attr("class","fa fa-refresh fa-spin")
wait.exit().remove();ViewPane.call(chart)
wait.remove()}
...
return panel}}(snow,d3))
var snow=snow||{};(function(S,d3){S.chart=S.chart||{};S.chart.barplot=function(){var height=200
var width=200
var barwidth=20
var gap=2
var data={}
...
return chart}
chart.send=function(x){if(!arguments.length){return send}else{send=x;return chart;}}
chart.data=function(x){if(!arguments.length){return data}else{data=x;return chart;}}
chart.width=function(x){if(!arguments.length){return width}else{width=x;return chart;}}
chart.barwidth=function(x){if(!arguments.length){return barwidth}else{barwidth=x;return chart;}}
chart.height=function(x){if(!arguments.length){return height}else{height=x;return chart;}}
return chart}}(snow,d3));var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3){S.chart.fs=function(){var pwd=""
var prefix="/f/"
var callback=function(d){console.log(d)}
var chart=function(selection){selection.classed("box-info",true)
...
ul.enter().append("ul")
var ls=function(dir){pwd=dir
Pwd.html(dir)
d3.json(prefix+dir,function(d){var li=ul.selectAll("li").data(d)
li.enter().append("li")
li.html(function(d){if(d.IsDir){return'<a><i class="fa fa-folder-o"></i> '+d.Name+"</a>"}
else{return'<a><i class="fa fa-file-o"></i> '+d.Name+"</a>"}})
li.on("click",function(d){if(d.IsDir){ls(pwd+"/"+d.Name)}
else{callback({"method":"POST","data":JSON.stringify({file:pwd+"/"+d.Name})})}})
li.exit().remove()})}
var buttons=[{"txt":"up","func":function(){var p=pwd.split("/")
var dir=pwd;if(p.length>0){p.pop()
dir=p.join("/")}
ls(dir)}},{"txt":"root","func":function(){ls("")}}]
var Btns=BtnDiv.selectAll("button").data(buttons)
Btns.enter().append("input").attr("type","button")
Btns.attr("value",function(d){return d.txt}).text(function(d){return d.txt})
Btns.on("click",function(d){d.func()})
ls(pwd)}
chart.prefix=function(x){if(!arguments.length){return prefix}else{prefix=x
return chart}}
chart.callback=function(x){if(!arguments.length){return callback}else{callback=x
return chart}}
return chart}}(snow,d3))
var snow=snow||{};(function(S,d3){S.chart=S.chart||{};S.chart.heatmap=function(){var data={};var cols;var rows;var rowHeight=7;var colWidth=12;var width=300;var height=500;var svg;var norm="row"
...
var sum=rows.map(function(x,i){var s=0.0;cols.forEach(function(y,j){s=s+parseFloat(data.table.rows[x].c[y].v)})
return s;})
...
return chart}}(snow,d3))
var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3){S.chart.icelist=function(){var db;var send=function(d){console.log(d)}
var chart=function(selection){var ul=selection.selectAll("ul").data([{}])
ul.enter().append("ul")
var list=function(){d3.json("/list/"+db,function(error,d){var lis=ul.selectAll("li").data(d);console.log(d)
lis.enter().append("li")
lis.selectAll("a").remove()
lis.append("a").attr("class","name").text(function(d){console.log(d);return d})
lis.on("click",function(d){send({"db":db,"id":d})})
lis.exit().remove()})}
list(db)}
chart.db=function(x){if(!arguments.length){return db}else{db=x;return chart}}
chart.send=function(x){if(!arguments.length){return send}else{send=x;return chart}}
return chart}}(snow,d3))
var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3){S.chart.labels=function(){var data
var send
var ul
...
return chart}}(snow,d3))
var snow=snow||{};(function(S,d3){S.chart.list=function(){var height=300
var width=500
var data={}
var col=0
var rows=[]
...
li.exit().remove()
li.enter().append("li")
li.text(function(d){return data.table.rows[d].c[col].v}).on("click",function(d){send(d)});}
chart.send=function(d){if(!arguments.length){return send}
else{send=d;return chart;}}
chart.rows=function(d){if(!arguments.length){return rows}
else{rows=d;return chart;}}
...
return chart}}(snow,d3))
var snow=snow||{};snow.canvas=snow.canvas||{};snow.chart=snow.chart||{};(function(d3,S){S.canvas.colorbar=function(){var scale
var width=200
var height=20
var chart=function(selection){var canvas=selection.selectAll("canvas").data([0])
canvas.exit().remove();canvas.enter().append("canvas")
canvas.attr("height",height+40).attr("width",width+40)
var ctx=canvas.node().getContext("2d");var grd=ctx.createLinearGradient(0,0,width+20,0);grd.addColorStop(0,scale.range()[0]);grd.addColorStop(1,scale.range()[1]);ctx.fillStyle=grd;ctx.fillRect(20,20,width,height);}
...
var scaled=false;var color=d3.scale.linear().range(["green","white","red"]).domain([-0.1,0,0.1])
var xScale
var yScale
...
controlDiv.exit().remove();var canvasDiv=self.selectAll(".canvasdiv").data([0])
canvasDiv.enter().append("div").attr("class","canvasdiv")
canvasDiv.exit().remove()
var canvas=canvasDiv.selectAll("canvas").data([0]);canvas.enter().append("canvas")
canvas.attr("width",xLen()*cellSize).attr("height",yLen()*cellSize).style("position","absolute")
...
svg.exit().remove()
var fc=Math.pow(2,foldLevel);yScale=d3.scale.linear().domain([0,yLen()*fc]).range([0,yLen()*cellSize])
xScale=d3.scale.linear().domain([0,xLen()*fc]).range([0,xLen()*cellSize])
var brushCb=function(){var extent=brush.extent();send({"extent":extent,"cellSize":cellSize,"xLen":xLen()*fc,"yLen":yLen()*fc});}
var brush=d3.svg.brush().x(xScale).y(yScale).on("brush",brushCb);svg.call(brush)}
...
var dims;var chart=function(selection){var row=selection;row.selectAll("*").remove();var self=row.selectAll("div").data([id])
...
var simple_render=function(){var wait=self.selectAll(".overlay").data([0])
wait.enter().append("div")
wait.attr("class","overlay").append("i").attr("class","fa fa-refresh fa-spin")
wait.exit().remove();var cb=function(){wait.remove();}
self.selectAll(".body").remove();var c=self.append("div").attr("class","body")
var colorbar=S.canvas.colorbar().scale(d3.scale.linear().domain(domain).range([color1,color2]))
...
d3.text("/get/"+dbname+"/"+id+"/"+pluginName+"/"+"mat"+fL,function(tsv){data=d3.tsv.parseRows(tsv)
...
return chart;}}
return chart}}(d3,snow))
var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3){S.chart.scatter=function(cb){var width=280;var height=280;var padding=20;var leftPadding=80;var scale=1.0;var data={}
...
return chart;};}(snow,d3))
var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3,$){S.chart.table=function(){var caption=""
var data={}
var chart=function(selection){selection.selectAll(".dataTables_wrapper").remove();var table=selection.selectAll("table").data([data])
...
return chart}}(snow,d3,jQuery))
var snow=snow||{};(function(S,d3){S.fig=function(){var data={};var core=function(i,d){console.log("core function TODO.")}
var layoutFormat=function(d){return"col-md-"+d}
var box=function(selection){selection.each(function(d,i){var div=d3.select(this);if(d.layout){div.classed(layoutFormat(d.layout),true)}else{div.classed(layoutFormat(12),true)}
...
if(d.title&&d.panel){if(d.panel.Title){d.panel.Title(d.title)}}
if(d.panel){if(d.data){body.call(d.panel)}else{body.call(d.panel)}}})}
