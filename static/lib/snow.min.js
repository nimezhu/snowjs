...
return a}
S.panel.general=function(cb){var ViewPane,CfgPane;var Content;var NavTabs;var NavView,NavCfg;var Config;var Div;var Uid;var Title=""
...
chart.barwidth=function(x){if(!arguments.length){return barwidth}
else{barwidth=x;return chart;}}
chart.height=function(x){if(!arguments.length){return height}
else{height=x;return chart;}}
return chart}}(snow,d3))
var snow=snow||{};snow.chart=snow.chart||{};(function(S,d3){S.chart.fs=function(){var pwd=""
var prefix="/f/"
...
li.exit().remove()})}
var buttons=[{"txt":"up","func":function(){let p=pwd.split("/")
let dir=pwd;if(p.length>0){p.pop()
dir=p.join("/")}
...
var chart=function(selection){var ul=selection.selectAll("ul").data([{}]).enter().append("ul")
...
var niceScale=function(){let max=data[0][0]
let min=data[0][0]
...
size=Math.max(r,c)
let l=size
let level=0;while(1){if(l<figSize){cellSize=Math.floor(figSize/l)
break;}
else{l=l/2
fL+=1}}}
