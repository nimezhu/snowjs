...
return chart;}}
chart.logscale=function(x){var domain=color.domain()
...
color.domain=function(d){if(!arguments.length){return c.domain()}else{var pdomain=[]
domain.forEach(function(d){pdomain.push(d+1.0)})
c.domain(pdomain)}}
color.range=function(d){if(!arguments.length){return c.range()}else{c.range(d)}}}
...
var mat=S.canvas.matrix().cellSize(cellSize).id(id).data(matrix).foldLevel(fL).send(send).domain(domain).range([color1,color2]);if(logChecked){mat.logscale()}
m.call(mat);cb();}
