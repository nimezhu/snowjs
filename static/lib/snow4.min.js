...
var extent=[e[0].map(xScale.invert),e[1].map(yScale.invert)];var rects=svg.selectAll(".resp").data([[e[0],e[0]],[e[1],e[1]],[e[1],e[0]]])
rects.enter().append("rect").attr("class","resp").merge(rects).attrs(function(d){return{"x":d[0][0],"y":d[0][1],"width":d[0][1]-d[0][0],"height":d[1][1]-d[1][0],"fill":"yellow","opacity":0.5}})
send({"extent":extent,"cellSize":cellSize,"xLen":xLen()*fc,"yLen":yLen()*fc});}
var brush=d3.brush().on("brush",brushCb);svg.call(brush)}
