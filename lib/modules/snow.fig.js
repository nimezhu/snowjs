  ...
  S.fig = function(){
    ...
    var box = function(selection){
      selection.each(function(d,i){
        ...
        let body = div.append("div").classed("tab-nav-custom",true)
        if (d.class) {
        ...
        */
		...
		if (d.panel) {
          if (d.data) {
            if (typeof d.data === typeof [1,2,3]) {
              var divs = body.selectAll("div").data(d.data);
              divs.exit().remove()
              divs.enter().append("div");
              divs.classed("tab-nav-custom",true).call(d.panel)
            } else {
              body.call(d.panel)
            }
          } else {
