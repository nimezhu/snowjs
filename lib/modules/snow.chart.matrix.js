...
(function(d3, S) {
  S.canvas.colorbar = function() {
    ...
    var chart = function(selection) {
      ...
      canvas.attr("height", height + 40).attr("width", width + 40)
  ...
  S.canvas.matrix = function() {
    ...
    var data = {}
    var scaled = false;
    var color = d3.scale.linear().range(["green", "white", "red"]).domain([-0.1, 0, 0.1])
    var xScale
    var yScale
    var chart = function(selection) {
      var self = selection;
      ...
      .selectAll("canvas").data([0]);
      canvas.enter().append("canvas")
      canvas.attr("width", xLen() * cellSize)
      ...
      yScale = d3.scale.linear().domain([0, yLen()*fc]).range([0, yLen() * cellSize])
      xScale = d3.scale.linear().domain([0, xLen()*fc]).range([0, xLen() * cellSize])
    ...
    chart.logscale = function(x) {
      ...
      if (x==true) {
        ...
        var c = d3.scale.log().domain(pdomain).range(range)
        color = function(d) {
          if(d<0.0) {
            return c(1.0)
          } else {
  ...
  S.chart.matrix = function() {
    ...
    var chart = function(selection) {
      ...
      self.enter().append("div")
      self.style("height", height+"px").style("width",width+"px").style("overflow", "scroll")
      self.exit().remove()
	  /*
      ...
      */
	  ...
	  header.enter().append("h3").classed("header",true)
	  header.text(function(d){return d})
      ...
      var simple_render = function() {
        ...
        var dColorbar = c.selectAll(".colorbar").data([0])
        dColorbar.enter().append("div").attr('class','colorbar')
        dColorbar.call(colorbar)
        ...
        m.enter().append("div").attr("class", "mat")
