...
(function(d3, S) {
  ...
  S.canvas.matrix = function() {
    ...
    var chart = function(selection) {
      ...
      var controlDiv = self.selectAll(".ctrl").data([0])
      controlDiv.enter().append("div").attr("class","ctrl")
      controlDiv.exit().remove();
      var canvasDiv = self.selectAll(".canvasdiv").data([0])
      canvasDiv.enter().append("div").attr("class","canvasdiv")
      canvasDiv.exit().remove()
      var canvas = canvasDiv
      .selectAll("canvas").data([0]);
      canvas.enter().append("canvas").merge(canvas)
      ...
      var brush = d3.svg.brush().x(xScale).y(yScale).on("brush", brushCb);
      svg.call(brush)
  ...
  S.chart.matrix = function() {
    ...
    var chart = function(selection) {
      ...
      var self = row.selectAll("div").data([id])
      self.enter().append("div").merge(self).style("height", height+"px").style("width",width+"px").style("overflow", "scroll")
      self.exit().remove()
      console.log(self);
	  /*
      ...
      */
	  var header = self.selectAll(".header").data([id])
	  header.enter().append("h3").classed("header",true).merge(header).text(function(d){return d})

      var simple_render = function() {
        var wait = self.selectAll(".overlay").data([0])
        wait.enter().append("div").merge(wait).attr("class", "overlay").append("i").attr("class", "fa fa-refresh fa-spin")
        wait.exit().remove();
        var cb = function() {
          wait.remove();
        }
        ...
        var c = self.selectAll(".body").data([0])
        c.enter().append("div").merge(c).attr("class", "body")
        console.log(c);
        ...
        var dColorbar = c.selectAll(".colorbar").data([1])
        dColorbar.enter().append("div").attr('class','colorbar').merge(dColorbar).call(colorbar)

        var infodiv = c.selectAll(".info").data([0])
          if (cellSize==1) {p=""} else {p="s"}
        infodiv.enter().append("div").attr("class","info")
        .merge(infodiv).text("each cell width is "+ cellSize + " pixel"+p+" and represents " + Math.pow(2,fL) + " data point"+p+".")
        var m = c.selectAll(".mat").data([0])
        ...
        .range([color1, color2])
        .logscale(logscale)
        m.enter().append("div").attr("class", "mat").merge(m).call(mat)
        cb();
      }
