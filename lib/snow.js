var snow = snow || {};
snow.tools = snow.tools || {};
snow.canvas = snow.canvas || {};
snow.panel = snow.panel || {};
snow.gsheet = snow.gsheet || {};
snow.chart = snow.chart || {};
(function(S,d3){
	/*
	S.canvas.scatter = function() {
	var chart = function(selection){

}
return chart;
}
/* scatter with config tab */
...
S.chart.scatter = function(cb) {
	...
	var initScaleTypeDomain = function(type,column) { //column zero index.
		...
		if (type=="log") {
			...
			//console.log(groups)
			//console.log(Object.keys(groups).sort())
	...
	var chart = function(selection) {
		var xScale=initScaleTypeDomain(xScaleType || scaleType,x).range([0,width])
		...
		svg.exit().remove()
		svg.enter().append("svg").attr("height",height+2*padding)
		...
		var brushCb = function() {
			...
			var dots=svg.selectAll(".dots").data([0])
			dots.enter()
			.append("g")
			.attr("class","dots")
			.attr("transform","translate("+leftPadding+","+padding+")")
			...
			vdots.attr("id",function(d,i){return "n"+i})
...
S.gsheet.query = function(sql, key,callback){
...
S.config = function() {
	...
	var parseMap = {
		...
		"a":function(s){return JSON.parse("["+s.value+"]")},
	...
	var chart = function(selection) {
		...
		tm.exit().remove()
		tm.enter().append("div")
		var tc = tm.selectAll(".checklist").data(function(d){ //tc for checkbox
			return data[d].m //default for selections.
		})
		tc.exit().remove()
		tc.enter().append("div").attr("class","checklist")
		tc.selectAll("input").remove();
		var checkboxs = tc.append("input")
			.attr("type","checkbox")
		...
		tc.selectAll("text").remove();
		tc.append("text").text(function(d){return d})
		...
		button.on("click",function(e){
			...
			tc.selectAll('input:checked').attr("selected",function(d){
